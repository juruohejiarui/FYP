...
一轮默认只承载**一条**实质：一个问题，或一个短答，或一条建议，或一种药，或一个征象，或一个解释。计划行只能挂**一个**约束编号，或回应轮写 `none`。半句 echo（2–6 字）不算第二条，允许「半句接住 + 推进」且 ≤ 二十。同一条约束的「短答 + 极短限定」才可同轮，不得把两条独立约束焊在一轮。SETS 组成员各说一轮，中间插对方短确认，禁止收成「注意生活」。短确认必须过下面的承上选词，不能一律写 `对`。

- **承上**：对方刚说完实质，本轮优先带上一轮已出口的 2–4 字，或一个接得上的短确认，再问、答或给下一条。不要光秃秃 `嗯` 就换无关主题。清单第二条前单独短确认仍够。
- **启下**：半句 echo 不得单独占一轮后停住；字数够则同轮接上问/答/建议（`夜里口干，那查过啥没有？`）；拆轮后自己下一轮必须推进。禁止整句回读和「你是问能不能查」类元复述。
- **回应问句**：上一轮是患者问句时，医生首个实质轮必须答、澄清、为答而追问，或先收住再换题。不能把无关约束直接当答案。

#### 承上选词：句类和角色都要接得上

承上不是随便塞 `对` / `嗯` / echo。每一处短确认或半句接住必须同时过两关：

1. **句类接得上**：承上词要接上一轮的言语行为，不能只看「有没有回应」。
   - 陈述 / 症状 / 病史：可用 `对`、短 echo、`嗯`。
   - 建议 / 嘱咐 / 做法：用 `嗯` / `行` / `好` / `知道了`，不用 `对`。
   - 解释 / 判断 / 许可：用 `哦` / `这样啊` / `明白了`，不用 `对`。
   - 问句：要答、澄清或为答而追问；不能拿确认词当答案。
   echo 只能带对方已出口的 2–4 字，且能嵌进本轮句法；禁止整句回读。
1. **角色接得上**：医患知识不对称。患者不裁定医生的建议或医学判断是否「对」；医生不把患者主诉当成一条指令来「好」。人设可以换词，不能推翻这层不对称。`researched_layperson` / `medical_worker_or_student` 也一样。

清单第二条前单独 `嗯` 仍够。不要为了「有承上」就一律写 `对`。

**反 1** 医生：「少吃点辣。」患者：「对。」
**正 1** 医生：「少吃点辣。」患者：「嗯。」或「哦，行。」
解释：建议是嘱咐，不是让患者验真的命题。`对` 像在批改医生，角色越权，句类也不接。

**反 2** 患者：「夜里老口干。」医生：「好。」
**正 2** 患者：「夜里老口干。」医生：「夜里口干，查过没有？」
解释：主诉是陈述，不是指令。医生说 `好` 像在批准发言或领任务。应带已出口短词再推进。
...
- 对方刚说完实质，本行优先承上（2–4 字已出口词或接得上的短确认）再启下。短确认必须过承上选词：建议/解释后患者不用 `对`，主诉后医生不用 `好`。`echo_then_probe` 不得空转停住。
...
T2 | 患者 | pure_backchannel | none | reacts_to=T1 | very_short | 嗯或行 | 对；偷渡新事实
